...
export default function BartOrders() {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.status === 'CANCELLED') {
        toast.success(t('bartOrders.toastCancelSuccess'));
        closeCancelModal();
      } else if (variables.status === 'PREPARING') toast.info(t('bartOrders.toastPrepSuccess'));
      else if (variables.status === 'READY') toast.success(t('bartOrders.toastReadySuccess'));
      else if (variables.status === 'COMPLETED') toast.success(t('bartOrders.toastCompSuccess'));
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.error || t('bartOrders.toastErrorDefault'));
    }
  });
  ...
  const handleCancelSubmit = () => {
    ...
    if (cancelOrder.status !== 'NEW') {
      if (cancelReason.trim().length < 5) {
        toast.error(t('bartOrders.reasonMinLength'));
        return;
      }
      if (user?.role !== 'ADMIN' && adminCode.trim() === '') {
        toast.error(t('bartOrders.adminCodeRequired'));
        return;
      }
    }
  ...
  const getElapsedTime = (createdAt: string) => {
    ...
    if (minutes < 1) return t('bartOrders.timeNow');
    return t('bartOrders.timeMins', { mins: minutes });
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-amber-500">
        ...
        <h2 className="text-xl font-bold text-slate-800">{t('bartOrders.loading')}</h2>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-red-500">
        ...
        <h2 className="text-2xl font-black text-slate-900">{t('bartOrders.errorTitle')}</h2>
        <p className="text-slate-500">{t('bartOrders.errorDesc')}</p>
      </div>
    );
  }
  ...
  const OrderCard = ({ order, accentColor, nextStatus, nextLabel, NextIcon }: any) => (
    <div className={`bg-white rounded-2xl shadow-sm border-l-4 ${accentColor} p-5 flex flex-col gap-4 transition-all hover:shadow-md animate-in fade-in slide-in-from-bottom-2 relative group`}>
      ...
      <button
        ...
        title={t('bartOrders.cancelBtnTitle')}
      >
      ...
      <div className="flex justify-between items-start pr-10">
        <div>
          ...
          <h3 className="font-black text-slate-900 text-lg leading-tight">
            {order.user?.fullName || t('bartOrders.unknownCustomer')}
          </h3>
        </div>
        <div className="flex items-center gap-1.5 bg-slate-100 text-slate-600 px-2.5 py-1 rounded-md text-sm font-bold">
          <Clock className="w-4 h-4" />
          <span className={getElapsedTime(order.createdAt) !== t('bartOrders.timeNow') && parseInt(getElapsedTime(order.createdAt)) > 10 ? 'text-red-500 animate-pulse' : ''}>
    ...
    <div className="max-w-[1600px] mx-auto min-h-[85vh] flex flex-col pt-4 pb-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-black text-slate-900 flex items-center gap-3">
            ...
            {t('bartOrders.dashboardTitle')}
          </h1>
          <p className="text-slate-500 font-medium mt-1">
            {t('bartOrders.dashboardDesc')}
          </p>
        </div>
        <div className="flex items-center gap-2 text-sm font-bold text-slate-400 bg-white px-4 py-2 rounded-full shadow-sm border border-slate-100">
          ...
          {isFetching ? t('bartOrders.syncing') : t('bartOrders.syncActive')}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start flex-grow">
        ...
        <div className="bg-slate-100/50 rounded-3xl p-4 flex flex-col border border-slate-200 h-[calc(100vh-220px)] min-h-[500px]">
          <div className="flex items-center justify-between mb-4 px-2 shrink-0">
            <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-blue-500"></span> {t('bartOrders.colNew')}
            </h2>
          ...
          <div className="flex flex-col gap-4 overflow-y-auto pr-2 pb-4 flex-grow">
            {newOrders.map(order => <OrderCard key={order.id} order={order} accentColor="border-blue-500" nextStatus="PREPARING" nextLabel={t('bartOrders.actionStartPrep')} NextIcon={Play} />)}
            {newOrders.length === 0 && <p className="text-center text-slate-400 font-medium py-8 italic">{t('bartOrders.emptyNew')}</p>}
          </div>
        </div>
        ...
        <div className="bg-slate-100/50 rounded-3xl p-4 flex flex-col border border-slate-200 h-[calc(100vh-220px)] min-h-[500px]">
          <div className="flex items-center justify-between mb-4 px-2 shrink-0">
            <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-amber-500 animate-pulse"></span> {t('bartOrders.colPrep')}
            </h2>
            <span className="bg-white text-slate-900 font-black px-3 py-1 rounded-full shadow-sm text-sm">{preparingOrders.length}</span>
          </div>
          <div className="flex flex-col gap-4 overflow-y-auto pr-2 pb-4 flex-grow">
            {preparingOrders.map(order => <OrderCard key={order.id} order={order} accentColor="border-amber-500" nextStatus="READY" nextLabel={t('bartOrders.actionMarkReady')} NextIcon={CheckCircle2} />)}
            {preparingOrders.length === 0 && <p className="text-center text-slate-400 font-medium py-8 italic">{t('bartOrders.emptyPrep')}</p>}
          </div>
        </div>
        ...
        <div className="bg-slate-100/50 rounded-3xl p-4 flex flex-col border border-slate-200 h-[calc(100vh-220px)] min-h-[500px]">
          <div className="flex items-center justify-between mb-4 px-2 shrink-0">
            <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-emerald-500"></span> {t('bartOrders.colReady')}
            </h2>
            <span className="bg-white text-slate-900 font-black px-3 py-1 rounded-full shadow-sm text-sm">{readyOrders.length}</span>
          </div>
          <div className="flex flex-col gap-4 overflow-y-auto pr-2 pb-4 flex-grow">
            {readyOrders.map(order => <OrderCard key={order.id} order={order} accentColor="border-emerald-500" nextStatus="COMPLETED" nextLabel={t('bartOrders.actionComplete')} NextIcon={Check} />)}
            {readyOrders.length === 0 && <p className="text-center text-slate-400 font-medium py-8 italic">{t('bartOrders.emptyReady')}</p>}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-300">
            ...
            <div className="p-6 space-y-6">
              {cancelOrder.status !== 'NEW' && (
                <div className="bg-red-50 border border-red-200 p-4 rounded-2xl flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-red-900">{t('bartOrders.strictCancelTitle')}</h4>
                    <p className="text-sm text-red-700 mt-1">
                      {t('bartOrders.strictCancelDesc')}
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">
                    {t('bartOrders.cancelReasonLabel')} {cancelOrder.status !== 'NEW' && <span className="text-red-500">*</span>}
                  </label>
                  <textarea
                    value={cancelReason}
                    onChange={(e) => setCancelReason(e.target.value)}
                    placeholder={t('bartOrders.cancelReasonPlaceholder')}
                ...
                {cancelOrder.status !== 'NEW' && user?.role !== 'ADMIN' && (
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">
                      {t('bartOrders.adminCodeLabel')} <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="password"
                      value={adminCode}
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
              ...
              >
                {updateStatusMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : t('bartOrders.confirmCancelBtn')}
              </button>
            </div>
